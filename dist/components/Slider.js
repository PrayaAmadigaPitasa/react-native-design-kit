...
export default function Slider({ containerStyle, minValue = 0, minTrackContainerStyle, maxValue = 100, maxTrackContainerStyle, initialValue = 25, button, buttonValue, trackContainerStyle, indicator, numberOfSection = 10, numberOfSubSection = 2, onChangeValue, }) {
    ...
    function getIndicatorComponent() {
        ...
        if (numberOfSection > 0) {
            for (let index = 0; index <= numberOfSection; index++) {
                indicatorComponent.push(React.createElement(View, { key: `{indicatorSection: ${index}}`, style: styles.indicator }));
                if (index < numberOfSection) {
                    for (let indexSub = 0; indexSub < numberOfSubSection - 1; indexSub++) {
                        indicatorComponent.push(React.createElement(View, { key: `{indicator: ${index}, sub: ${indexSub}}`, style: StyleSheet.flatten([
                                styles.indicator,
                                styles.indicatorSub,
                            ]) }));
                    }
    ...
    return (React.createElement(View, { style: StyleSheet.flatten([styles.container, containerStyle]) },
        button && (React.createElement(TouchableOpacity, { activeOpacity: 0.5, onPress: () => setValue(getValue() -
                (buttonValue !== undefined
                    ? Math.max(0, buttonValue)
                    : (maxValue - minValue) * 0.15)) },
            React.createElement(View, { style: styles.buttonLeftIconContainer },
                React.createElement(Icon, { style: styles.buttonIcon, name: "caret-left" })))),
        width === undefined ? (React.createElement(View, { style: styles.trackDefaultBaseContainer },
            React.createElement(View, { onLayout: event => setWidth(event.nativeEvent.layout.width), style: trackContainerStyle }))) : (React.createElement(View, { style: styles.sectionTrackContainer },
            indicator && (React.createElement(View, { style: StyleSheet.flatten([
                    styles.sectionIndicator,
                    styles.sectionIndicatorTop,
                ]) }, getIndicatorComponent())),
            React.createElement(View, { style: styles.sectionTrack },
                ...
                React.createElement(View, { onLayout: event => {
                    ...
                    }, onStartShouldSetResponder: () => true, onResponderStart: event => {
                        setPageX(event.nativeEvent.pageX), setStartProgress(progress);
                    }, onResponderMove: event => {
                        if (pageX !== undefined) {
                            setProgress(Math.max(0, Math.min(1, startProgress +
        ...
        button && (React.createElement(TouchableOpacity, { activeOpacity: 0.5, onPress: () => setValue(getValue() +
                (buttonValue !== undefined
                    ? Math.max(0, buttonValue)
                    : (maxValue - minValue) * 0.15)) },
            React.createElement(View, { style: styles.buttonRightIconContainer },
                React.createElement(Icon, { style: styles.buttonIcon, name: "caret-right" }))))));
}
